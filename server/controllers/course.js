...
//If the title is //React Course for beginners//
...
export const create = async (req, res) => {
  ...
  try {
    ...
    if (alreadyExist)
      return res
        .status(400)
        .send("Course title already in use. Please enter an unique title.");
    ...
    console.log(err);
    return res.status(400).send("Course create failed. Please Try Again.");
  }
};
...
export const removeVideo = async (req, res) => {
  try {
    ...
    // console.log("Video Removal====>",req.body);
    // return;
...
//to update the existing courses
...
export const checkEnrollment = async (req, res) => {
  const { courseId } = req.params;
  //find the courses of the currently logged in user
...
export const paidEnrollment = async (req, res) => {
  try {
    ...
    const course = await Course.findById(req.params.courseId)
      ...
      .exec();
    if (!course.paid) return;
    //application fee 30% [3% stripe charge] platform fee 70% goes to instructor
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      //charge subscriber remaining balance to seller(Platform Fee)
      payment_intent_data: {
